...
`go-ruby-graphql/graphql` reimplements Ruby's `graphql` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-graphql/graphql` directly, with no Ruby runtime;
